...
function Search() {
    ...
    useEffect(() => {
        ...
        fetch(`http://127.0.0.1:8000/visitor/search?${queryParams.toString()}`)
            .then((res) => res.json())
            .then((data) => {
                setLogs(data);
                setIsLoading(false);
    ...
    useEffect(() => {
        ...
        if (selectedVisitor.type === "known") {
            queryParams.append("user_id", selectedVisitor.id);
        } else {
            queryParams.append("unknown_visitor_id", selectedVisitor.id);
        }
